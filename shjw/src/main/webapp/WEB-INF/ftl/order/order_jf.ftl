...
<body>
<div class="top">
      <p><img src="${basePath}/img/ico_qiz.png"/>当前位置：订单管理>已付款订单>缴费确认<span id="clocks">2017-08-01 11：29 星期二</span></p>
       ...
       <a href="${basePath}/order/order_yynj.shtml">
              ...
              <#else>预约年检</#if></a>
        <a href="${basePath}/order/order_jfqr.shtml" class="curr">
...
<div class="down">
     <div class="search">
           <div><input value="${jf_search?default('')}" placeholder="输入车主姓名或车牌号进行查询" class="search_value"/><a style="cursor:pointer" class="search_jf">查询</a></div><a class="jq-search" style="cursor:pointer">精确查询<i class="ico-jt ico-jt-1"></i></a>
           ...
           <#else>
              <div class="cx-result" style="display:block">
                  ...
                  <form  action="${basePath}/order/order_jfqr.shtml?status=1" method="post">
                  <div class="choose">                           
        ...
        <table width="100%" border="0">
			<tr>
             ...
             <#if jflist?exists && jflist?size gt 0>
             <#list jflist as order>
                 <tr>
                    ...
                    <td><a style='cursor:pointer;width:100%;text-align:center' onclick='jfqr_btn(${order.orderid})'>缴费确认</a>
                    <a style='cursor:pointer;width:100%;text-align:center'  onclick='js_print(${order.orderid})'>打印</a></td>
                  </tr>
            </#list>
             <#else>
				 <tr>
			        <td class="text-center danger" colspan="15">未找到待缴费订单 </td>
				</tr>
			    </#if> 
                </table>
     ...
     <ul class="page"  id="page">
       <li><a href="${basePath}/order/order_jfqr.shtml?pageIndex=1">首页</a></li>
       <li>
       <#if pageIndex-1=0>
            <a href="${basePath}/order/order_jfqr.shtml?pageIndex=1">上一页</a>
       <#else>
            <a href="${basePath}/order/order_jfqr.shtml?pageIndex=${pageIndex-1}">上一页</a>
       </#if> 
       </li>    
      
              <#if total<5 && total gt 0>            
              <#list 1..total as i>    
          <li id="${i}"><a href="${basePath}/order/order_jfqr.shtml?pageIndex=${i}">${i}</a></li>
              </#list>
             <#elseif total=0>
            <li id="1"><a href="${basePath}/order/order_jfqr.shtml?pageIndex=1">1</a></li>
              <#else>
                     <#if pageIndex<5>
                         <#list 1..4 as i>    
                         <li id="${i}"><a href="${basePath}/order/order_jfqr.shtml?pageIndex=${i}">${i}</a></li>
             ...
             <li>  
               ...
               <a href="${basePath}/order/order_jfqr.shtml?pageIndex=${pageIndex}">下一页</a>
              <#else>
              <a href="${basePath}/order/order_jfqr.shtml?pageIndex=${pageIndex+1}">下一页</a>
             </#if>
             </li>
         <#if total=0>
          <li><a href="${basePath}/order/order_jfqr.shtml?pageIndex=${total+1}">末页</a></li>
          <#else>
          <li><a href="${basePath}/order/order_jfqr.shtml?pageIndex=${total}">末页</a></li>          
          </#if>
       </ul>
         <#else>
           <p style="font-size:16px;color:#FF8040;text-align:center">成功为您找到${jflist?size}款符合的订单</p>  
       </#if>        
</div>
...
<div class="popbox1" style="height:90%;overflow-x:hidden;overflow-y:auto;position:absolute;top:5%">
	<div class="mainlist">
	  <h2>打印凭据<a class="close-btn1" style='cursor:pointer'>&times;</a></h2>
		<ul>
...
$(document).ready(function(){
   ...
   $('.search_jf').click(function(){
       var jf_search=$('.search_value').val();   
       location.href="${basePath}/order/order_jfqr.shtml?jf_search="+encodeURI(encodeURI(jf_search));           
   });
   
   $('.shuaxin').click(function(){
       location.href="${basePath}/order/order_jfqr.shtml";
   });
...
<script>
 //缴费确认 
    function jfqr_btn(orderid){
           var data={orderid:orderid};    
           location.href="${basePath}/pay/fukuan.shtml?orderid="+orderid+"&status=1";
         /*$.ajax({
		     url:"${basePath}/order/order_jfqr_nj.shtml",
		     data:data,			 
		     type:"post",			        		        
		     success:function(result){
	            layer.msg("缴费确认进入年检");        
