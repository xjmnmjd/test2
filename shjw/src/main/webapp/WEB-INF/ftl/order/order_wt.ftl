...
<body>
 <div class="top">
       <p><img src="${basePath}/img/ico_qiz.png"/>当前位置：订单管理>已付款订单>委托办理<span id="clocks">2017-08-01 11：29 星期二</span></p>
       ...
       <a href="${basePath}/order/order_zlsj.shtml" >
             ...
             <#else>资料收集 </#if></a>
       <a href="${basePath}/order/order_wtbl.shtml" class="curr">
...
<div class="down">
   <div class="search">
                <div><input value="${wt_search?default('')}" placeholder="输入车主姓名或车牌号进行查询" class="search_value"/><a style="cursor:pointer" class="search_wt">查询</a></div>
          ...
          <form  action="${basePath}/order/order_wtbl.shtml?status=1" method="post">
               <div class="choose">                           
            ...
            <table width="100%" border="0">
             ...
             <#list wtlist as order>
                 <tr>
                    ...
                    <td><a style='cursor:pointer;width:100%;text-align:center' onclick='wtbl_btn(${order.orderid})'>委托办理</a>           
                    <a style='cursor:pointer;width:100%;text-align:center' onclick='chargeback(${order.orderid})'>退单</a></td>   
                  </tr>
            </#list>
             <#else>
				 <tr>
			        <td class="text-center danger" colspan="15">未找到委托办理订单</td>
				</tr>
			    </#if> 
                </table>
      ...
      <ul class="page"  id="page">
       <li><a href="${basePath}/order/order_wtbl.shtml?pageIndex=1">首页</a></li>
       <li>
       <#if pageIndex-1=0>
            <a href="${basePath}/order/order_wtbl.shtml?pageIndex=1">上一页</a>
       <#else>
            <a href="${basePath}/order/order_wtbl.shtml?pageIndex=${pageIndex-1}">上一页</a>
       </#if> 
       </li>    
      
                <#if total<5 && total gt 0>            
              <#list 1..total as i>    
          <li id="${i}"><a href="${basePath}/order/order_wtbl.shtml?pageIndex=${i}">${i}</a></li>
              </#list>
           <#elseif total=0>
            <li id="1"><a href="${basePath}/order/order_wtbl.shtml?pageIndex=1">1</a></li>
              <#else>
                     <#if pageIndex<6>
                         <#list 1..5 as i>    
                         <li id="${i}"><a href="${basePath}/order/order_wtbl.shtml?pageIndex=${i}">${i}</a></li>
             ...
             <li>  
               ...
               <a href="${basePath}/order/order_wtbl.shtml?pageIndex=${pageIndex}">下一页</a>
              <#else>
              <a href="${basePath}/order/order_wtbl.shtml?pageIndex=${pageIndex+1}">下一页</a>
             </#if>
             </li>
         <#if total=0>
          <li><a href="${basePath}/order/order_wtbl.shtml?pageIndex=${total+1}">末页</a></li>
          <#else>
          <li><a href="${basePath}/order/order_wtbl.shtml?pageIndex=${total}">末页</a></li>          
          </#if>    
       </ul> 
          <#else>
           <p style="font-size:16px;color:#FF8040;text-align:center">成功为您找到${wtlist?size}款符合的订单</p>
       </#if>         
 </div>
<!--委托办理弹出层开始-->
<div id="screen"></div>
<div class="popbox">
	<div class="mainlist">
        <h2>委托办理<a class="close-btn" style='cursor:pointer'>&times;</a></h2>
        <input style="display:none" class="wt_orderid"/>
        <a class="choose-kf wz" style="margin-top:40px"><i class="ico-radio ico-radio-1" id="zlyj"></i>资料已寄</a>
        <a class="choose-kf wz"><i class="ico-radio ico-radio-1" id="zlsd"></i>资料收到</a>
        <a class="choose-kf wz"><i class="ico-radio ico-radio-1" id="zlbz"></i>资料不足退回</a>
        <a class="choose-kf wz" style="margin-bottom:40px"><i class="ico-radio ico-radio-1" id="wzth"></i>违章退回</a>
        <div class="btn"><div><a class="close-btn" style='cursor:pointer'>关闭</a>
        <a class="wtbl_ok_btn" style='cursor:pointer;margin-left:15px'>确定</a></div></div>
	</div>
</div>
<!--弹出层结束-->
...
$(document).ready(function(){
   ...
   $('.search_wt').click(function(){
       var wt_search=$('.search_value').val();   
       location.href="${basePath}/order/order_wtbl.shtml?wt_search="+encodeURI(encodeURI(wt_search));           
   });
      $('.shuaxin').click(function(){
    location.href="${basePath}/order/order_wtbl.shtml";	             
   });
...
<script type="text/javascript">
//委托办理弹出 
function wtbl_btn(orderid){
	var h = $(document).height();
	$('#screen').css({ 'height': h });	
	$('#screen').show();
	$('.popbox').center();
	$('.popbox').fadeIn();
	$('.wt_orderid').val(orderid);
	return false;  
}
//退单弹出 
...
$(document).ready(function(){
	...
	//委托办理定
    $('.wtbl_ok_btn').click(function(){           
         var orderid=$('.wt_orderid').val();
         var data={orderid:orderid};                   
         //资料已寄
         if($('#zlyj').hasClass('ico-radio ico-radio-2')){
            $.ajax({
		       url:"${basePath}/order/order_zlyj.shtml",
		       data:data,			 
		       type:"post",			        		        
		       success:function(result){
	       	     layer.msg("资料已寄等待中");
	       	     $('.popbox').fadeOut(function(){ $('#screen').hide(); });
	             setTimeout(function(){window.location.href="${basePath}/order/order_wtbl.shtml?pageIndex="+$('.page_i').val();},2000);
		       }
	        });  
         }
         //资料收到
         if($('#zlsd').hasClass('ico-radio ico-radio-2')){
            $.ajax({
		     url:"${basePath}/order/order_zlsd.shtml",
		     data:data,			 
		     type:"post",			        		        
		     success:function(result){
	       	  layer.msg("资料收到缴费");
       ...
       // 资料不足  
       if($('#zlbz').hasClass('ico-radio ico-radio-2')){
         $.ajax({
		   url:"${basePath}/order/order_zlbz.shtml",
		   data:data,			 
		   type:"post",			        		        
		    success:function(result){
	          layer.msg("资料不足去收集资料");
       ...
       // 违章退回
       if($('#wzth').hasClass('ico-radio ico-radio-2')){
           var status=1;
