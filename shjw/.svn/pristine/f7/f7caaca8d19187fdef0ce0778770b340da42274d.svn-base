	...
	MobileCalendar.prototype = {
		init : function(params) {
			...
			if (this.trigger.getAttribute("data-lcalendar") != null) {
				var arr = this.trigger.getAttribute("data-lcalendar")
						.split(',');
		...
		bindEvent : function(type) {
			...
			function popupDate(e) {
				...
				_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">'
						...
						+ '<div class="date_grid"></div>' + '</div>' + '</div>'
						+ '</div>' + '<div class="date_btn_box">'
						+ '<div class="date_btn lcalendar_cancel">取消</div>'
						+ '<div class="date_btn lcalendar_finish">确定</div>'
						+ '</div>' + '</div>';
				document.body.appendChild(_self.gearDate);
				dateCtrlInit();
				var lcalendar_cancel = _self.gearDate
						.querySelector(".lcalendar_cancel");
				lcalendar_cancel.addEventListener('click',
						closeMobileCalendar);
				var lcalendar_finish = _self.gearDate
						.querySelector(".lcalendar_finish");
				lcalendar_finish.addEventListener('click',
						finishMobileDate);
			...
			function dateCtrlInit() {
				...
				_self.gearDate.querySelector(".date_yy").setAttribute("val",
						dateArr.yy);
				_self.gearDate.querySelector(".date_mm").setAttribute("val",
						dateArr.mm);
				_self.gearDate.querySelector(".date_dd").setAttribute("val",
						dateArr.dd);
				setDateGearTooth();
			}
			// 呼出年月插件
			function popupYM(e) {
				...
				_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">'
						...
						+ '<div>'
						+ '<div class="gear date_mm" data-datetype="date_mm"></div>'
						+ '<div class="date_grid">' + '<div>月</div>' + '</div>'
						+ '</div>' + '</div>' + '</div>' + '</div>';
				document.body.appendChild(_self.gearDate);
				ymCtrlInit();
				var lcalendar_cancel = _self.gearDate
			...
			function popupDateTime(e) {
				...
				_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">'
						...
						+ '<div class="date_grid">' + '<div>分</div>' + '</div>'
						+ '</div>' + '</div>' + // date_roll
						'</div>' + // date_roll_mask
						'</div>';
				document.body.appendChild(_self.gearDate);
			...
			function dateTimeCtrlInit() {
				...
				if (/^\d{4}-\d{1,2}-\d{1,2}\s\d{2}:\d{2}$/
						.test(_self.trigger.value)) {
				...
				_self.gearDate.querySelector(".date_dd").setAttribute("val",
						dateArr.dd);
				setDateGearTooth();
				_self.gearDate.querySelector(".time_hh").setAttribute("val",
						dateArr.hh);
				_self.gearDate.querySelector(".time_mm").setAttribute("val",
						dateArr.mi);
				setTimeGearTooth();
			}
			// 呼出时间插件
			function popupTime(e) {
				...
				var lcalendar_cancel = _self.gearDate
						.querySelector(".lcalendar_cancel");
				lcalendar_cancel.addEventListener('touchstart',
						closeMobileCalendar);
				var lcalendar_finish = _self.gearDate
						.querySelector(".lcalendar_finish");
				lcalendar_finish.addEventListener('touchstart',
						finishMobileTime);
				var time_hh = _self.gearDate.querySelector(".time_hh");
				var time_mm = _self.gearDate.querySelector(".time_mm");
				time_hh.addEventListener('touchstart', gearTouchStart);
			...
			function setDateGearTooth() {
				...
				if (date_yy && date_yy.getAttribute("val")) {
					...
					for (var p = 0; p <= passY - 1; p++) {
						itemStr += "<div class='tooth'>" + (_self.minY + p)
								+ "</div>";
					}
					date_yy.innerHTML = itemStr;
					var top = Math
							.floor(parseFloat(date_yy.getAttribute('top')));
					if (!isNaN(top)) {
						top % 2 == 0 ? (top = top) : (top = top + 1);
						top > 8 && (top = 8);
						var minTop = 8 - (passY - 1) * 2;
						top < minTop && (top = minTop);
						date_yy.style["-webkit-transform"] = 'translate3d(0,'
								+ top + 'em,0)';
						date_yy.setAttribute('top', top + 'em');
						yyVal = Math.abs(top - 8) / 2;
						date_yy.setAttribute("val", yyVal);
						$('.date_title .year').text(
								$($('.date_yy .tooth')[yyVal]).text());
					} else {
						date_yy.style["-webkit-transform"] = 'translate3d(0,'
								+ (8 - yyVal * 2) + 'em,0)';
				...
				if (date_mm && date_mm.getAttribute("val")) {
					...
					for (var p = 0; p < maxM - minM + 1; p++) {
						itemStr += "<div class='tooth'>" + (minM + p + 1)
								+ "</div>";
					}
					...
					$('.date_title .month').text(
							$($('.date_mm .tooth')[mmVal]).text());
					date_mm.style["-webkit-transform"] = 'translate3d(0,'
							+ (8 - (mmVal - minM) * 2) + 'em,0)';
				...
				if (date_dd && date_dd.getAttribute("val")) {
					...
					for (var p = 0; p < maxD - minD + 1; p++) {
						itemStr += "<div class='tooth'>" + (minD + p + 1)
								+ "</div>";
					}
					...
					date_dd.style["-webkit-transform"] = 'translate3d(0,'
							+ (8 - (ddVal - minD) * 2) + 'em,0)';
			...
			function setTimeGearTooth() {
				...
				if (time_hh && time_hh.getAttribute("val")) {
					...
					time_hh.innerHTML = i;
					time_hh.style["-webkit-transform"] = 'translate3d(0,'
							+ (8 - hhVal * 2) + 'em,0)';
				...
				if (time_mm && time_mm.getAttribute("val")) {
					...
					time_mm.innerHTML = i;
					time_mm.style["-webkit-transform"] = 'translate3d(0,'
							+ (8 - mmVal * 2) + 'em,0)';
			...
			function finishMobileDate(e) {
				var passY = _self.maxY - _self.minY + 1;
				var date_yy = parseInt(Math.round(_self.gearDate.querySelector(
						".date_yy").getAttribute("val")));
				var date_mm = parseInt(Math.round(_self.gearDate.querySelector(
						".date_mm").getAttribute("val"))) + 1;
				date_mm = date_mm > 9 ? date_mm : '0' + date_mm;
				var date_dd = parseInt(Math.round(_self.gearDate.querySelector(
						".date_dd").getAttribute("val"))) + 1;
			...
			function finishMobileDateTime(e) {
				...
				var date_dd = parseInt(Math.round(_self.gearDate.querySelector(
						".date_dd").getAttribute("val"))) + 1;
				date_dd = date_dd > 9 ? date_dd : '0' + date_dd;
				var time_hh = parseInt(Math.round(_self.gearDate.querySelector(
						".time_hh").getAttribute("val")));
				time_hh = time_hh > 9 ? time_hh : '0' + time_hh;
				var time_mm = parseInt(Math.round(_self.gearDate.querySelector(
						".time_mm").getAttribute("val")));
				time_mm = time_mm > 9 ? time_mm : '0' + time_mm;
				_self.trigger.value = (date_yy % passY + _self.minY) + "-"
						+ date_mm + "-" + date_dd + " "
						+ (time_hh.length < 2 ? "0" : "") + time_hh
						+ (time_mm.length < 2 ? ":0" : ":") + time_mm;
				closeMobileCalendar(e);
			}
			// 时间确认
