...
<body>
        <div class="top">
            <p><img src="${basePath}/img/ico_qiz.png"/>当前位置：订单管理>已结算订单<span id="clocks">2017-08-01 11：29 星期二</span></p>
             <a style="width:120px"  href="${basePath}/order/sp_order.shtml"><#if sp_count gt 0>超权限审批<span class="ts">(${sp_count})</span>
            <#else>超权限审批</#if></a>
             <a style="width:120px" href="${basePath}/order/month_list.shtml">当月新进订单</a>
             <a class="curr" style="width:120px">已结算订单</a>  
    </div>
        <div class="down">
            <div class="search">
              <div><input value="${a_search?default('')}" placeholder="输入车主姓名或车牌号进行查询" class="search_value"/><a style="cursor:pointer" class="search_js">查询</a></div><a class="jq-search" style="cursor:pointer">精确查询<i class="ico-jt ico-jt-1"></i></a>
          ...
          <form  action="${basePath}/order/month_list.shtml?status=1" method="post">
            ...
            <table width="100%" border="0">
                  ...
                  <#if acount_list?exists &&acount_list?size gt 0>
               <#list acount_list as order>
                 <tr>                
                ...
                <#else>
				 <tr>
			        <td class="text-center danger" colspan="15">未找到已结算订单 </td>
				</tr>
			    </#if> 
    ...
    <#if s_status==0>
     <ul class="page"  id="page">
       ...
       <li><a href="${basePath}/order/account_list.shtml?pageIndex=1">首页</a></li>
       <li>
       <#if pageIndex-1=0>
            <a href="${basePath}/order/account_list.shtml?pageIndex=1">上一页</a>
       <#else>
            <a href="${basePath}/order/account_list.shtml?pageIndex=${pageIndex-1}">上一页</a>
       </#if> 
       </li>    
      
            <#if total<5 && total gt 0>          
              <#list 1..total as i>    
          <li id="${i}"><a href="${basePath}/order/account_list.shtml?pageIndex=${i}">${i}</a></li>
              </#list>
             <#elseif total=0>
            <li id="1"><a href="${basePath}/order/account_list.shtml?pageIndex=1">1</a></li>
              <#else>
                     <#if pageIndex<5>
                         <#list 1..4 as i>    
                         <li id="${i}"><a href="${basePath}/order/account_list.shtml?pageIndex=${i}">${i}</a></li>
             ...
             <li>  
               ...
               <a href="${basePath}/order/account_list.shtml?pageIndex=${pageIndex}">下一页</a>
              <#else>
              <a href="${basePath}/order/account_list.shtml?pageIndex=${pageIndex+1}">下一页</a>
             </#if>
             </li>
         <#if total=0>
          <li><a href="${basePath}/order/account_list.shtml?pageIndex=${total+1}">末页</a></li>
          <#else>
          <li><a href="${basePath}/order/account_list.shtml?pageIndex=${total}">末页</a></li>          
          </#if>             
          </ul>
         <#else>
           <p style="font-size:16px;color:#FF8040;text-align:center">成功为您找到${acount_list?size}款符合的订单</p>
        </#if>           
      </div>  
...
$(function(){
    $('.search_js').click(function(){
       ...
       location.href="${basePath}/order/account_list.shtml?m_search="+encodeURI(encodeURI(m_search));           
   });
      $('.shuaxin').click(function(){
      location.href="${basePath}/order/account_list.shtml";
